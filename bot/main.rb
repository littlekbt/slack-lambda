...
EOF
  ...
  def extract_action(text)
    m = text.match(/<@U7337SHAR(?:\|slack-lambda)?>\s*(register|show|list|remove|run|exec|help)/)
  ...
  class UserFile
    ...
    SHOW_REGEXP     = /<@U7337SHAR(?:\|slack-lambda)?>\s*show\s*([a-z|A-Z|0-9|_|-]*)/
    LIST_REGEXP     = /<@U7337SHAR(?:\|slack-lambda)?>\s*list/
    REMOVE_REGEXP     = /<@U7337SHAR(?:\|slack-lambda)?>\s*remove\s*([a-z|A-Z|0-9|_|-]*)/
    RUN_REGEXP      = /<@U7337SHAR(?:\|slack-lambda)?>\s*run\s*([a-z|A-Z|0-9|_|-]*)/
    class << self
      def register(text)
